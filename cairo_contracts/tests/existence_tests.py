...
@pytest.mark.asyncio
async def test_nested_array(contract_factory):
    starknet, contract, owner_acc = contract_factory

    print(int("ETHUSD".encode('utf-8').hex()))
    # await contract.test_nested_array(
    #     "ETHUSD".encode('utf-8').hex(),
    # ).invoke()

    # res = await contract.read_nested_array().call()
