...
from deposits_withdraws.deposits.deposit_utils import (
    Deposit,
    get_deposit_notes,
...
func main{output_ptr, pedersen_ptr : HashBuiltin*, range_check_ptr, ecdsa_ptr : SignatureBuiltin*}(
    ...
    %{ deposits = program_input["deposits_data"] %}
    ...
    %{
        ...
        t1 = time.time()
    %}
    execute_deposit_transaction{note_dict=note_dict}()
    execute_deposit_transaction{note_dict=note_dict}()
    execute_deposit_transaction{note_dict=note_dict}()
    execute_deposit_transaction{note_dict=note_dict}()
    execute_deposit_transaction{note_dict=note_dict}()
...
func execute_deposit_transaction{
    ...
    %{
        ...
        memory[ids.deposit.address_ + TOKEN_OFFSET] = on_chain_deposit_info["token"]
        memory[ids.deposit.address_ + AMOUNT_OFFSET] = on_chain_deposit_info["amount"]
        memory[ids.deposit.address_ + ADDRESS_PK_OFFSET] = on_chain_deposit_info["stark_key"]
    %}
