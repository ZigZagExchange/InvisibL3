...
func verify_withdraw_notes{
    ...
    let (withdraw_hash : felt) = withdraw_tx_hash(
        note_hashes_len, note_hashes, withdrawal.address_pk
    )
...
func verify_signatures{pedersen_ptr : HashBuiltin*}(
    ...
    %{
        ...
        ids.signature_r = sig[0]
        ids.signature_s = sig[1]
    %}
...
func withdraw_tx_hash{pedersen_ptr : HashBuiltin*}(
    note_hashes_len : felt, note_hashes : felt*, withdraw_address_stark_key : felt
) -> (res):
    let hash_ptr = pedersen_ptr
    with hash_ptr:
        let (hash_state_ptr) = hash_init()
        let (hash_state_ptr) = hash_update(hash_state_ptr, note_hashes, note_hashes_len)
        let (hash_state_ptr) = hash_update_single(hash_state_ptr, withdraw_address_stark_key)
...
func handle_inputs(notes_len : felt*, notes : Note**, refund_note : Note*):
    %{
        ...
        withdraw_notes = current_withdrawal["notes"]

        memory[ids.notes_len] = len(withdraw_notes)
        memory[ids.notes] = notes = segments.add()
        for i, note in enumerate(withdraw_notes):
            memory[notes + i*NOTE_SIZE + ADDRESS_PK_OFFSET] = note["address_pk"]
            memory[notes + i*NOTE_SIZE + TOKEN_OFFSET] = on_chain_withdraw_info["token"]
            memory[notes + i*NOTE_SIZE + AMOUNT_OFFSET] = note["amount"]
            memory[notes + i*NOTE_SIZE + BLINDING_FACTOR_OFFSET] = note["blinding_factor"]
            memory[notes + i*NOTE_SIZE + INDEX_OFFSET] = note["index"]

        # REFUND NOTE ==============================

        refund_note__ = current_withdrawal["refund_note"]
        memory[ids.refund_note.address_ + ADDRESS_PK_OFFSET] = refund_note__["address_pk"]
        memory[ids.refund_note.address_ + TOKEN_OFFSET] = refund_note__["token"]
        memory[ids.refund_note.address_ + AMOUNT_OFFSET] = refund_note__["amount"]
        memory[ids.refund_note.address_ + BLINDING_FACTOR_OFFSET] = refund_note__["blinding_factor"]
        memory[ids.refund_note.address_ + INDEX_OFFSET] = refund_note__["index"]
    %}
